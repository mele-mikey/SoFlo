...
fn review_grammar_text_blocking(app: tauri::AppHandle, model_path: String, text: String, quick: bool) -> CommandResult<String> {
    ...
    let system_instruction = if quick {
        ...
        "You are SoFlo's rigorous academic writing reviewer. Return only one complete valid JSON array: no Markdown, no code fences, and no commentary outside the array. Return 3 to 8 high-confidence, useful objects whenever the input contains a complete rough-draft paragraph; do not return [] merely because spelling is acceptable. Each object must have string keys kind, original, replacement, reason, category, partOfSpeech, definition, useCase, alternatives, and synonyms. kind must be mechanic, style, or structure. alternatives and synonyms must each be arrays of at most three strings. First include clear mechanics for spelling, apostrophes, capitalization, hyphenation, duplicated spaces, and punctuation. Then actively find several specific formal academic improvements. Prioritize weak sentence starters and openers, conversational or vague phrases, weak verbs, transitions, short closing phrases, fragments, run-ons, unclear conclusions, and needless wordiness. Give several distinct style suggestions when a draft is deliberately rough. Every style object must use kind style, and both its original and replacement must be an exact, meaningful 1 to 9 word excerpt; never rewrite a full sentence. Style replacements must preserve meaning while making the wording more formal, precise, and academic. A mechanic original, replacement, and alternative must be one to three words or a short punctuation/space sequence. Include at most one structure suggestion and only if changing paragraph order or adding a transition genuinely improves the argument. Never invent facts, alter citations, flag proper names merely for being unfamiliar, or make empty thesaurus substitutions. Explain the concrete reason each replacement improves the writing. original must exactly match input text, including punctuation and capitalization. For word corrections when useful, provide part of speech, a brief definition, a short use case, and related synonyms. Use empty strings and [] only when those fields do not apply."
    };
    ...
    let response = client.post(format!("http://127.0.0.1:{}/v1/chat/completions", AI_SERVER_PORT)).json(&serde_json::json!({
        ...
        "max_tokens": if quick { 1000 } else { 3600 },
        "temperature": 0.0
