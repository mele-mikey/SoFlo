...
fn review_grammar_text_blocking(app: tauri::AppHandle, model_path: String, text: String, quick: bool) -> CommandResult<String> {
    ...
    let system_instruction = if quick {
        "You are a precise academic spelling and mechanics checker. Return only valid JSON: an array of at most 20 objects with string keys kind, original, replacement, reason, category, partOfSpeech, definition, useCase, alternatives, and synonyms. kind must always be mechanic. alternatives and synonyms must each be arrays of at most three strings. Find only high-confidence spelling, apostrophe, capitalization, hyphenation, duplicated-space, and basic punctuation errors. Every original, replacement, and alternative must be one to three words at most, or a short punctuation/space sequence. The original must be an exact continuous excerpt from the input. Keep the replacement minimal. For a misspelled word, provide the most likely correction as replacement and up to three plausible spelling alternatives in alternatives. Never rewrite a sentence, suggest a style or sentence-fluidity change, alter the writer's voice, flag citations, or flag a proper name merely because it is unfamiliar. Explain why the correction is needed in reason. For word corrections, provide the corrected word's part of speech, a brief definition, a short use case, and up to three related synonyms when useful; use empty strings and [] for mechanics where those do not apply. Return [] when there are no clear errors."
    } else {
        "You are a careful academic writing editor. Return only valid JSON: an array of at most 10 objects with string keys kind, original, replacement, reason, category, partOfSpeech, definition, useCase, alternatives, and synonyms. kind must be mechanic, style, or structure. alternatives and synonyms must each be arrays of at most three strings. First include high-confidence mechanic corrections for spelling, apostrophes, capitalization, hyphenation, duplicated spaces, and punctuation; their original, replacement, and alternatives must be one to three words or a short punctuation/space sequence. Then include a small number of style suggestions that make academic writing clearer, more formal, and more precise, paying particular attention to sentence starters, transitions, topic sentences, and conclusions. Style suggestions must use an exact continuous excerpt of at most 48 words and a polished replacement that preserves the writer's meaning and stays under 48 words. Finally, include at most two structure suggestions only when paragraph order or transitions clearly weakens the argument; use the first sentence of the relevant paragraph as original and describe the proposed move or transition in replacement. Never invent facts, alter citations, flag proper names merely for being unfamiliar, or make empty thesaurus substitutions. Explain the reasoning for every suggestion. Provide part of speech, definition, use case, and up to three related synonyms for word corrections when useful; use empty strings and [] when they do not apply."
    };
    ...
    let response = client.post(format!("http://127.0.0.1:{}/v1/chat/completions", AI_SERVER_PORT)).json(&serde_json::json!({
        ...
        "max_tokens": if quick { 720 } else { 3000 },
        "temperature": 0.1
