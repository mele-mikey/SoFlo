...
function App() {
  ...
  return <div className={`app theme-${library.settings.themeColor} ${library.settings.reduceMotion ? 'reduce-motion' : ''}`}>
    ...
    {modal?.type === 'aiSet' && <AiSetDialog onClose={() => setModal(null)} onCreate={(request) => generateAiSet(modal.classId, request)} />}
...
function AiSetDialog({ onClose, onCreate }: { onClose: () => void; onCreate: (request: AiSetRequest) => Promise<string | null> }) {
  ...
  const create = async () => { if (!hasMaterial) return; setCreating(true); setError(''); const failure = await onCreate({ sources, pasted, topic: '', guidance, title, cardCount, depth }); if (failure) { setError(failure); setCreating(false) } }
  return <div className="paper-dialog-backdrop" role="presentation"><section className="paper-dialog ai-set-dialog ai-set-dialog-simple" role="dialog" aria-modal="true" aria-label="Create flashcards with AI"><header><div><p className="eyebrow">LOCAL AI</p><h2>Create flashcards</h2></div><button className="icon-button" disabled={creating} onClick={onClose} aria-label="Close"><X size={17} /></button></header><form onSubmit={(event) => { event.preventDefault(); void create() }}><div className="paper-dialog-content"><p>Paste notes, a study guide, or simply describe a topic. Adding files is optional. Your class syllabus is included as supporting context when one is available.</p><label>Set name<input disabled={creating} value={title} onChange={(event) => setTitle(event.target.value)} /></label><label>What do you want to study?<textarea disabled={creating} autoFocus rows={7} value={pasted} onChange={(event) => setPasted(event.target.value)} placeholder="Paste notes here, or write something like: Create cards for introductory Java inheritance." /></label><label>Study instructions<textarea disabled={creating} rows={3} value={guidance} onChange={(event) => setGuidance(event.target.value)} placeholder="For math: make more practice questions with different numbers, slightly harder when it makes sense." /></label><div className="ai-set-options"><label>Coverage<select disabled={creating} value={cardCount} onChange={(event) => setCardCount(event.target.value === 'auto' ? 'auto' : Number(event.target.value) as 10 | 20 | 30)}><option value="auto">Comprehensive (up to 100)</option><option value="10">About 10 cards</option><option value="20">About 20 cards</option><option value="30">About 30 cards</option></select></label><label>Depth<select disabled={creating} value={depth} onChange={(event) => setDepth(event.target.value as AiSetRequest['depth'])}><option value="quick">Quick</option><option value="standard">Standard</option><option value="detailed">Detailed</option></select></label></div><section className="ai-source-section"><strong>Have files instead?</strong><span>Optionally add up to five PDFs, Word documents, or PowerPoints (.pptx). SoFlo automatically keeps math problems and equations in problem-and-answer form.</span><button type="button" disabled={creating} className="button button-quiet ai-action" onClick={() => void choose()}><Plus size={15} /> Add files ({sources.length}/5)</button>{sources.length > 0 && <div className="ai-source-list">{sources.map((source) => <span key={source}>{source.split(/[\\/]/).pop()}<button type="button" disabled={creating} onClick={() => setSources((current) => current.filter((item) => item !== source))}><X size={13} /></button></span>)}</div>}</section>{error && <p className="study-web-picker-error" role="alert">{error}</p>}</div><footer><button type="button" className="button button-quiet" disabled={creating} onClick={onClose}>Cancel</button><button className="button button-primary ai-action" disabled={!hasMaterial || creating}>{creating ? 'Creating flashcards…' : 'Create flashcards'}</button></footer></form></section></div>
}
