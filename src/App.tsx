...
function AiSetDialog({ onClose, onCreate }: { onClose: () => void; onCreate: (request: AiSetRequest) => Promise<string | null> }) {
  ...
  const create = async () => { if (!hasMaterial) return; setCreating(true); setError(''); const failure = await onCreate({ sources, pasted, topic: '', guidance: '', title, cardCount: 'auto', depth: 'standard' }); if (failure) { setError(failure); setCreating(false) } }
