import { Menu, NotebookPen, Plus, Search, Sparkles, X } from 'lucide-react'
...
type AiSetRequest = { sources: string[]; lectureIds: string[]; pasted: string; topic: string; guidance: string; title: string; cardCount: 'auto' | 10 | 20 | 30; depth: 'quick' | 'standard' | 'detailed' }
...
const remoteAiConfigured = (settings: AppSettings) => settings.onlineAiEnabled && /^https:\/\/[^\s]+$/i.test(settings.onlineAiEndpoint.trim()) && settings.onlineAiAccessKey.trim().length >= 32
const remoteAiModelReference = (settings: AppSettings) => `soflo-remote://${btoa(settings.onlineAiEndpoint.trim().replace(/\/+$/, ''))}.${btoa(settings.onlineAiAccessKey.trim())}`
...
function App() {
  ...
  const lectureSaveErrorNotified = useRef(false)
  ...
  useEffect(() => { let unlisten: (() => void) | undefined; void listen<UpdateDownloadProgress>('app-update-download-progress', (event) => setUpdateDownloadProgress(event.payload)).then((dispose) => { unlisten = dispose }); return () => unlisten?.() }, [])
  ...
  useEffect(() => {
    ...
    void listen<string>('lecture-content-updated', (event) => {
      // Never let an asynchronous recording/analysis update replace notes that
      // are still in the editor's save queue. The background result remains in
      // the lecture panel and will be loaded after the student's save finishes.
      if (activeLecture?.id === event.payload && pendingLecture.current?.id !== event.payload) void api.getLecture(event.payload).then(setActiveLecture).catch(() => undefined)
    }).then((dispose) => { unlisten = dispose })
    return () => unlisten?.()
  }, [activeLecture?.id])
  const refreshWordAiModelStatus = useCallback(async () => {
    if (library && library.settings.onlineAiEnabled) {
      const configured = remoteAiConfigured(library.settings)
      setGeneralAiModelReady(configured)
      setWordAiModelReady(configured)
      try { setVoiceAiModelReady(await api.voiceAiModelReady()) } catch { setVoiceAiModelReady(false) }
      setWordAiModelStatusLoaded(true)
      return
    }
    try {
  ...
  useEffect(() => { setWordAiModelStatusLoaded(false); void refreshWordAiModelStatus() }, [library?.settings.aiModelPath, library?.settings.aiVoiceModelPath, library?.settings.onlineAiEnabled, library?.settings.onlineAiEndpoint, library?.settings.onlineAiAccessKey, refreshWordAiModelStatus])
  useEffect(() => {
  ...
  const flushLecture = useCallback(async () => {
    ...
    try {
      ...
      setSaveState('saved')
      lectureSaveErrorNotified.current = false
    } catch (error) {
      setSaveState('error')
      pendingLecture.current = lectureToSave
      if (!lectureSaveErrorNotified.current) {
        lectureSaveErrorNotified.current = true
        showToast(error instanceof Error ? `Lecture save paused: ${error.message}` : 'Lecture save paused. SoFlo will keep trying; do not close the app.', 'error')
      }
      lectureSaveTimer.current = window.setTimeout(() => { void flushLecture() }, 2_000)
    }
  }, [showToast])
  ...
  const ensureAiModel = async () => {
    ...
    if (library.settings.onlineAiEnabled) {
      if (!remoteAiConfigured(library.settings)) throw new Error('Enter your HTTPS SoFlo Server endpoint and its device pairing key in Settings.')
      return remoteAiModelReference(library.settings)
    }
  ...
  const askStudyTutor = async (front: string, back: string, question: string, studentWork: string, message: string, history: string, cardsContext: string) => {
    const aiModelPath = await ensureAiModel()
    if (aiModelPath === null) throw new Error('Turn on AI in Settings to use the study tutor.')
    return api.askStudyTutor(aiModelPath, front, back, question, studentWork, message, history, cardsContext)
  }
  const generateStudyTutorPractice = async (front: string, back: string, question: string, history: string, cardsContext: string, request: string) => {
    const aiModelPath = await ensureAiModel()
    if (aiModelPath === null) throw new Error('Turn on AI in Settings to use the study tutor.')
    return api.generateStudyTutorPractice(aiModelPath, front, back, question, history, cardsContext, request)
  }
  ...
  const generateAiSet = async (targetClassId: string, request: AiSetRequest) => {
    try {
      const [imported, lecturePapers] = await Promise.all([
        Promise.all(request.sources.map((source) => /\.docx?$/i.test(source) ? api.importWordText(source) : /\.pptx$/i.test(source) ? api.importPowerPointText(source) : api.importPdfText(source))),
        Promise.all(request.lectureIds.map((lectureId) => api.getLecture(lectureId))),
      ])
      ...
      const textOnly = !request.sources.length && !lecturePapers.length && Boolean(manual)
      const materials = [
        request.topic.trim() && `TOPIC OR PROMPT:\n${request.topic.trim()}`,
        manual && `${textOnly ? 'TEXT OR TOPIC' : 'PASTED MATERIAL'}:\n${manual}`,
        ...imported.map((text, index) => `UPLOADED MATERIAL ${index + 1}:\n${text}`),
        ...lecturePapers.map((lecture, index) => `LECTURE PAPER ${index + 1} — ${lecture.title}:\n${lecture.contentPlain.trim() || lecture.content}`),
      ].filter(Boolean).join('\n\n--- NEXT SOURCE ---\n\n')
      if (!materials.trim()) throw new Error('Add a file, lecture paper, pasted study material, or a topic first.')
      ...
      const sourceKinds = [request.sources.length && 'uploaded material', request.lectureIds.length && 'lecture papers', request.pasted.trim() && 'pasted material', request.topic.trim() && 'a topic prompt'].filter(Boolean).join(', ')
  ...
  return <div className={`app theme-${library.settings.themeColor} ${library.settings.reduceMotion ? 'reduce-motion' : ''}`}>
    ...
    {modal?.type === 'aiSet' && <AiSetDialog classId={modal.classId} onClose={() => setModal(null)} onCreate={(request) => generateAiSet(modal.classId, request)} />}
    {availableUpdate && <div className="paper-dialog-backdrop" role="presentation"><section className="paper-dialog" role="dialog" aria-modal="true" aria-label="SoFlo update available"><header><div><p className="eyebrow">SOFLO UPDATE</p><h2>Version {availableUpdate.version} is ready</h2></div><button className="icon-button" disabled={downloadingUpdate} onClick={() => setAvailableUpdate(null)} aria-label="Close"><X size={17} /></button></header><div className="paper-dialog-content"><p>SoFlo downloads the installer first, then restarts only after the complete file is verified.</p>{downloadingUpdate && <div className="update-download-progress"><strong>{updateDownloadProgress?.message ?? 'Preparing update download…'}</strong>{updateDownloadProgress?.totalBytes ? <><div className="ai-download-track"><i style={{ width: `${updateDownloadProgress.percent ?? 0}%` }} /></div><span>{updateDownloadProgress.percent ?? 0}% · {(updateDownloadProgress.downloadedBytes / 1_048_576).toFixed(1)} MB of {(updateDownloadProgress.totalBytes / 1_048_576).toFixed(1)} MB</span></> : <span>{updateDownloadProgress?.downloadedBytes ? `${(updateDownloadProgress.downloadedBytes / 1_048_576).toFixed(1)} MB downloaded` : 'This may take a while on slow Wi-Fi. Keep SoFlo open.'}</span>}</div>}<p className="subtle-copy">This check only runs because you enabled update notifications. It is SoFlo&apos;s only automatic network request.</p></div><footer><button className="button button-quiet" disabled={downloadingUpdate} onClick={() => setAvailableUpdate(null)}>Not now</button><button className="button button-primary" disabled={downloadingUpdate} onClick={() => { setUpdateDownloadProgress(null); setDownloadingUpdate(true); void api.downloadAndLaunchAppUpdate(availableUpdate.version, availableUpdate.downloadUrl).catch((error) => { setDownloadingUpdate(false); showToast(error instanceof Error ? error.message : 'SoFlo could not download that update.', 'error') }) }}>{downloadingUpdate ? updateDownloadProgress?.percent !== null && updateDownloadProgress?.percent !== undefined ? `Downloading ${updateDownloadProgress.percent}%` : 'Downloading update...' : 'Download and restart'}</button></footer></section></div>}
    ...
    {aiConsentOpen && <div className="ai-consent-backdrop" role="presentation"><section className="ai-consent-card" role="dialog" aria-modal="true" aria-label="Use AI?"><p className="eyebrow">ARTIFICIAL INTELLIGENCE</p><h2>This action will use AI.</h2><p>{library.settings.onlineAiEnabled ? 'SoFlo will send this request over HTTPS to your paired SoFlo Server. The server is still yours; its Cloudflare URL alone cannot use it.' : 'SoFlo will download its local models once, then process this document on your PC.'} Research & Grade only sends a short topic query - never your paper - to Crossref for scholarly metadata. Turn AI off any time in Settings.</p><div><button className="button button-quiet" onClick={() => closeAiConsent(false)}>Return</button><button className="button button-primary" onClick={() => closeAiConsent(true)}>Proceed</button></div></section></div>}
...
function AiSetDialog({ classId, onClose, onCreate }: { classId: string; onClose: () => void; onCreate: (request: AiSetRequest) => Promise<string | null> }) {
  const [sources, setSources] = useState<string[]>([])
  const [lecturePapers, setLecturePapers] = useState<LectureSummary[]>([])
  const [lectureIds, setLectureIds] = useState<string[]>([])
  const [lecturesLoading, setLecturesLoading] = useState(true)
  const [pasted, setPasted] = useState('')
  const [guidance, setGuidance] = useState('')
  const [cardCount, setCardCount] = useState<AiSetRequest['cardCount']>('auto')
  const [depth, setDepth] = useState<AiSetRequest['depth']>('standard')
  ...
  useEffect(() => {
    ...
    void api.listLectures(classId).then((items) => { if (live) setLecturePapers(items) }).catch(() => { if (live) setLecturePapers([]) }).finally(() => { if (live) setLecturesLoading(false) })
    return () => { live = false }
  }, [classId])
  const choose = async () => {
    ...
    if (!picked) return
    setSources((current) => [...current, ...(Array.isArray(picked) ? picked : [picked])].filter((source, index, all) => all.indexOf(source) === index).slice(0, 5))
  }
  const toggleLecture = (lectureId: string) => setLectureIds((current) => current.includes(lectureId) ? current.filter((id) => id !== lectureId) : current.length < 5 ? [...current, lectureId] : current)
  const hasMaterial = Boolean(sources.length || lectureIds.length || pasted.trim())
  const create = async () => { if (!hasMaterial) return; setCreating(true); setError(''); const failure = await onCreate({ sources, lectureIds, pasted, topic: '', guidance, title, cardCount, depth }); if (failure) { setError(failure); setCreating(false) } }
  return <div className="paper-dialog-backdrop" role="presentation"><section className="paper-dialog ai-set-dialog ai-set-dialog-simple" role="dialog" aria-modal="true" aria-label="Create flashcards with AI"><header><div><p className="eyebrow">LOCAL AI</p><h2>Create flashcards</h2></div><button className="icon-button" disabled={creating} onClick={onClose} aria-label="Close"><X size={17} /></button></header><form onSubmit={(event) => { event.preventDefault(); void create() }}><div className="paper-dialog-content"><div className="ai-set-intro"><span><Sparkles size={17} /></span><p>Build a set from notes, uploads, and saved lecture papers. SoFlo reads each selected source in chunks, so long lecture papers stay usable.</p></div><label>Set name<input disabled={creating} value={title} onChange={(event) => setTitle(event.target.value)} /></label><label>What do you want to study?<textarea disabled={creating} autoFocus rows={6} value={pasted} onChange={(event) => setPasted(event.target.value)} placeholder="Paste notes here, or write something like: Create cards for introductory Java inheritance." /></label><label>Study instructions<textarea disabled={creating} rows={3} value={guidance} onChange={(event) => setGuidance(event.target.value)} placeholder="For math: make more practice questions with different numbers, slightly harder when it makes sense." /></label><div className="ai-set-options"><label>Coverage<select disabled={creating} value={cardCount} onChange={(event) => setCardCount(event.target.value === 'auto' ? 'auto' : Number(event.target.value) as 10 | 20 | 30)}><option value="auto">Comprehensive (up to 100)</option><option value="10">About 10 cards</option><option value="20">About 20 cards</option><option value="30">About 30 cards</option></select></label><label>Depth<select disabled={creating} value={depth} onChange={(event) => setDepth(event.target.value as AiSetRequest['depth'])}><option value="quick">Quick</option><option value="standard">Standard</option><option value="detailed">Detailed</option></select></label></div><div className="ai-source-grid"><section className="ai-source-section"><div className="ai-source-heading"><strong>Study files</strong><small>{sources.length}/5 selected</small></div><span>PDFs, Word documents, and PowerPoints.</span><button type="button" disabled={creating || sources.length >= 5} className="button button-quiet ai-action" onClick={() => void choose()}><Plus size={15} /> Add files</button>{sources.length > 0 && <div className="ai-source-list">{sources.map((source) => <span key={source}>{source.split(/[\\/]/).pop()}<button type="button" disabled={creating} onClick={() => setSources((current) => current.filter((item) => item !== source))} aria-label={`Remove ${source.split(/[\\/]/).pop()}`}><X size={13} /></button></span>)}</div>}</section><section className="ai-source-section lecture-source-section"><div className="ai-source-heading"><strong><NotebookPen size={14} /> Lecture papers</strong><small>{lectureIds.length}/5 selected</small></div><span>Select saved lecture papers from this class. Their complete written notes are included.</span>{lecturesLoading ? <p className="ai-source-status">Loading lecture papers…</p> : lecturePapers.length ? <div className="lecture-source-list">{lecturePapers.map((lecture) => { const selected = lectureIds.includes(lecture.id); return <button type="button" key={lecture.id} disabled={creating || (!selected && lectureIds.length >= 5)} className={selected ? 'selected' : ''} onClick={() => toggleLecture(lecture.id)}><i>{selected ? '✓' : ''}</i><span><strong>{lecture.title}</strong><small>{lecture.lectureDate}</small></span></button> })}</div> : <p className="ai-source-status">No saved lecture papers in this class yet.</p>}</section></div><small className="ai-source-limit">Choose up to 5 files and 5 lecture papers — 10 sources total.</small>{error && <p className="study-web-picker-error" role="alert">{error}</p>}</div><footer><button type="button" className="button button-quiet" disabled={creating} onClick={onClose}>Cancel</button><button className="button button-primary ai-action" disabled={!hasMaterial || creating}>{creating ? 'Creating flashcards…' : 'Create flashcards'}</button></footer></form></section></div>
}
