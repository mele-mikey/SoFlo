...
export function FlashcardSetEditor({
  ...
  useEffect(() => {
    ...
    setCards(set.cards);
    setDuplicateTitle(`${set.title} copy`);
  }, [set]);

  const persistSet = async () => {
    setSaving(true);
    try {
      const updated = await api.saveSet({
        id: set.id,
  ...
  const mastery = useMemo(
    () =>
      set.progress.reduce<Record<string, number>>((summary, item) => {
  ...
  const updateCard = (id: string, partial: Partial<Flashcard>) =>
    setCards((previous) =>
      previous.map((card) => (card.id === id ? { ...card, ...partial } : card)),
    );
  ...
  const exportCards = async () => {
    setMoreOpen(false);
    try {
      await api.exportFlashcardSetText(set.id);
      onToast("Flashcards exported to Downloads.");
    } catch (error) {
      onToast(
        error instanceof Error
          ? error.message
          : "Flashcards could not be exported.",
        "error",
      );
    }
  };
    ...
    <main className="set-editor">
      <header className="set-editor-header">
        <button className="back-button" onClick={onBack}>
          <ChevronLeft size={18} /> Flashcards
        </button>
        <div className="set-status">{saving ? "Saving…" : "Saved locally"}</div>
      </header>
      <section className="set-editor-main">
        <div className="set-heading">
          <span className="set-heading-icon">
            <BookOpen size={23} />
          </span>
          <div>
            <label className="set-title-field">
              ...
              <input
                ...
                onBlur={() => void persistSet()}
                onKeyDown={(event) => {
                  if (event.key === "Enter") event.currentTarget.blur();
                }}
                placeholder="Untitled flashcard set"
            ...
            <textarea
              ...
              aria-label="Flashcard set description"
              rows={1}
            />
          </div>
        </div>
        <div className="set-mastery-summary">
          <strong>
            {cards.length} {cards.length === 1 ? "Card" : "Cards"}
          </strong>
          <span>{mastery.mastered ?? 0} Mastered</span>
          <span>{mastery.familiar ?? 0} Familiar</span>
          <span>{mastery.learning ?? 0} Learning</span>
          <span>{mastery.needsWork ?? 0} Need work</span>
        </div>
        <div className="study-launcher">
          <span>Everything saves locally as you work.</span>
          <div>
            <button
              className="button button-primary button-small"
              onClick={() => onStudy("learn")}
              disabled={!cards.length}
            >
              <Sparkles size={15} /> Study
            </button>
            <span className="set-more">
              <button
                className="icon-button tiny"
                onClick={() => setMoreOpen((open) => !open)}
                aria-label="Flashcard set settings"
              >
                <MoreHorizontal size={17} />
              </button>
              {moreOpen && (
                <span className="paper-action-menu">
                  <button
                    onClick={() => {
                      setMoreOpen(false);
                  ...
                  <button onClick={() => void exportCards()}>
                  ...
                  <button
                    onClick={() => {
                      setMoreOpen(false);
                      void api
                        .setFlashcardSetStudyKind(
                          set.id,
                          set.studyKind === "math" ? "standard" : "math",
                        )
                        .then((updated) => {
                          onUpdated(updated);
                          onToast(
                            updated.studyKind === "math"
                              ? "Converted to a Math set."
                              : "Converted to a standard flashcard set.",
                          );
                        })
                        .catch(() => onToast("SoFlo could not change this set type.", "error"));
                    }}
                  >
                    <Sparkles size={14} /> {set.studyKind === "math" ? "Convert to standard set" : "Convert to Math set"}
                  </button>
                  <button
                    className="danger"
                    onClick={() => {
                  ...
                  {aiEnabled && (
                    <button
                      ...
                      disabled={!cards.length}
                    >
                      <Sparkles size={14} /> AI Convert to Study Web
                    </button>
                  )}
                </span>
              )}
            ...
            <div className="card-editor-list">
              {cards.map((card, index) => (
                <EditableCard
                  ...
                  onCreateNext={() => void addCard(true)}
                  onRemove={removeCard}
                />
              ))}
            </div>
        ...
        {!cards.length && (
          <div className="set-empty">
            <BookOpen size={25} />
            <h2>Start with your first card.</h2>
            <p>Use a term on the front and the explanation on the back.</p>
            <button
      ...
      {duplicateOpen && (
        <div className="paper-dialog-backdrop">
          <section
            ...
            role="dialog"
            aria-modal="true"
            aria-label="Duplicate flashcard set"
          >
            <header>
              <h2>Duplicate flashcard set</h2>
              <button
            ...
            <form
              onSubmit={(event) => {
                ...
                if (duplicateTitle.trim()) {
                  onDuplicate(duplicateTitle.trim());
...
function EditableCard({
  ...
  index: number;
  autoFocus: boolean;
  onChange: (id: string, partial: Partial<Flashcard>) => void;
  onPersist: (card: Flashcard) => Promise<void>;
  onCreateNext: () => void;
    ...
    >
      ...
      <label>
        ...
        <textarea
          ...
          onBlur={() => void onPersist(card)}
          onKeyDown={(event) => {
            if (event.key === "Tab" && !event.shiftKey)
              event.currentTarget
                .closest(".editable-card")
                ?.querySelector<HTMLTextAreaElement>("[data-card-back]")
                ?.focus();
          }}
          placeholder="Term"
          rows={2}
        ...
        <textarea
          ...
          onKeyDown={(event) => {
            if (event.key === "Enter" && !event.shiftKey) {
              event.preventDefault();
              void onPersist(card).then(onCreateNext);
            }
          }}
      ...
      <div className="card-actions">
        <button
          className={card.isStarred ? "card-action starred" : "card-action"}
          onClick={() => {
            const next = { ...card, isStarred: !card.isStarred };
            onChange(card.id, { isStarred: next.isStarred });
            void onPersist(next);
          }}
