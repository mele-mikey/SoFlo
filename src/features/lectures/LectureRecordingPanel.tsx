...
export function LectureRecordingPanel({ lectureId, aiEnabled, voiceModelReady, voiceModelPath, onEnsureVoiceModel, microphoneId, onMicrophoneChange, onNoteSuggestionsChange, onToast }: LectureRecordingPanelProps) {
  ...
  return <section className={`lecture-panel-shell panel-${panelVisibility}`}>
    ...
    {recording.state === 'analysis_failed' && <div className="lecture-recovery"><strong>Your lecture paper was kept safe</strong><p>{recording.statusMessage}</p>{aiEnabled && <button className="button button-primary button-small ai-action" disabled={busy} onClick={() => void retryAnalysis()}><Sparkles size={14} />Try organizing again</button>}</div>}
