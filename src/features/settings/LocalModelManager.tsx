...
const modelDetail: Record<ModelRole, { title: string; copy: string; memory: Record<ModelTier, string> }> = {
  ...
  writing: { title: 'Writing AI', copy: 'Grammar checks, word reference, and thesaurus', memory: { low: 'about 1–2 GB RAM', medium: 'about 3–5 GB RAM', high: 'about 5–8 GB RAM' } },
...
export function LocalModelManager({ settings, onClose, onSettingsChange, onModelsUpdated, onToast }: { settings: AppSettings; onClose: () => void; onSettingsChange: (settings: AppSettings) => void; onModelsUpdated: () => void; onToast: (message: string, type?: 'success' | 'error') => void }) {
  const [selection, setSelection] = useState<Record<ModelRole, ModelTier>>({
    ...
    writing: settings.aiWritingModelTier || 'medium',
  ...
  const apply = async () => {
    ...
    try {
      ...
      const aiWritingModelPath = await api.installAiModel('writing', selection.writing)
      const aiVoiceModelPath = await api.installAiModel('voice', selection.voice)
      const next = { ...settings, aiModelPath, aiWritingModelPath, aiVoiceModelPath, aiGeneralModelTier: selection.general, aiWritingModelTier: selection.writing, aiVoiceModelTier: selection.voice }
  ...
  const cleanup = async () => {
    setWorking('cleanup')
    try { await api.deleteUnusedAiModels(settings.aiModelPath, settings.aiWritingModelPath, settings.aiVoiceModelPath); onToast('Unused local models were removed.') }
    catch (error) { onToast(errorText(error), 'error') }
    finally { setWorking(null) }
  }
